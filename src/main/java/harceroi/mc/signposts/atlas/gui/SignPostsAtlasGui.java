...
public class SignPostsAtlasGui extends GuiComponent {
    ...
    private void updateAtlasData() {
        biomeData = AntiqueAtlasMod.atlasData.getAtlasData(stack, player.worldObj)
    ...
    public void drawScreen(int mouseX, int mouseY, float par3) {
        ...
        int mapStartX = MathUtil.roundToBase(
            (int) Math.floor(-((double) MAP_WIDTH / 2d + mapOffsetX + 2 * tileHalfSize) / mapScale / 16d),
            tile2ChunkScale);
        int mapStartZ = MathUtil.roundToBase(
            (int) Math.floor(-((double) MAP_HEIGHT / 2d + mapOffsetY + 2 * tileHalfSize) / mapScale / 16d),
            tile2ChunkScale);
        int mapEndX = MathUtil.roundToBase(
            (int) Math.ceil(((double) MAP_WIDTH / 2d - mapOffsetX + 2 * tileHalfSize) / mapScale / 16d),
            tile2ChunkScale);
        int mapEndZ = MathUtil.roundToBase(
            (int) Math.ceil(((double) MAP_HEIGHT / 2d - mapOffsetY + 2 * tileHalfSize) / mapScale / 16d),
            tile2ChunkScale);
        ...
        iter.setScope(
            new Rect().setOrigin(mapStartX, mapStartZ)
        ...
        float playerRotation = (float) Math.round(player.rotationYaw / 360f * PLAYER_ROTATION_STEPS)
    ...
    public void onGuiClosed() {
        ...
        PacketDispatcher.sendToServer(
            new BrowsingPositionPacket(stack.getItemDamage(), player.dimension, mapOffsetX, mapOffsetY, mapScale));
    }

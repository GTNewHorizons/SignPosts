...
@Mod(
    modid = SignPostsMod.ID,
    name = SignPostsMod.NAME,
    version = SignPostsMod.VERSION,
    dependencies = "required-after:antiqueatlas")
public class SignPostsMod {
    ...
    public void init(FMLInitializationEvent event) {
        ...
        IRecipe blockRecipe = new ShapedOreRecipe(
            new ItemStack(signPostBlock),
            new Object[] { ".WS", ".WS", ".W.", 'W', "plankWood", 'S', Items.sign });
    ...
    public static void addJumpTarget(int signX, int signY, int signZ, double jumpX, double jumpY, double jumpZ,
        String label, EntityPlayerMP player) {
        ...
        if (tileEntity instanceof SignPostTileEntity) {
            ...
            MarkerToTileMap.get(player.worldObj)
    ...
    public static void playerJump(int markerId, EntityPlayerMP player, String paymentHandlerKey) {
        ...
        int[] coords = MarkerToTileMap.get(player.worldObj)
            .getTileForMarker(markerId);
        // Get tile Entity
        if (coords != null) {
            ...
            if (jumpX != 0 && jumpY != 0 && jumpZ != 0) {
                ...
                if (player.capabilities.isCreativeMode
                    || paymentHandler.pay(player, (int) jumpX, (int) jumpY, (int) jumpZ)) {
                    // jump!
